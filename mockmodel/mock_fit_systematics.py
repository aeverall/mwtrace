...
if __name__=='__main__':
    ...
    message = f"""\n{run_id:03d} ---> Mock Systematics: {file}, Sample size: {size:d}, SF subset: {np.sum(sample['gaiasf_subset']):d}, SF ast subset: {np.sum(sample['astsf_subset']):d}
    ...
    with open(f'/data/asfe2/Projects/mwtrace_data/mockmodel/messages_sys.txt', 'a') as f:
        f.write(message)
        #pass
    print(message)
    ...
    nstep_all=5000
    save_G = sample['m'].copy()

    reserved={}
    for key in sample.keys():
        reserved[key] = sample[key].copy()

    for sys in ['z0', 'dust', 'Mto', 'gerr']:
        ...
        print('Sample size: ', np.sum(model_sys.sample['astsf_subset']))
        model_sys.sample['sf_subset'] = sample['astsf_subset'].copy()
        model_sys._generate_fid_pars(dr2_sf=dr3_sf, sub_sf=ast_sf, _m_grid=np.arange(0., 22.1, 0.1))
        ...
        # Optimize with BFGS
        model_sys.optimize_chunk(niter=10, ncores=80, label='sf_perr_bfgs', method='L-BFGS-B', verbose=True, minimize_options={'disp':False, 'ftol':1e-12, 'gtol':1e-7})
        # Run MCMC
        model_sys.mcmc(ncores=80, nsteps=nstep_all, label='sf_perr_mcmc', optimize_label='sf_perr_bfgs')
        # Save results
